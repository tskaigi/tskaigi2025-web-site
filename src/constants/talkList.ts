...
export const talkList: Talk[] = [
  {
    ...
    overview: `
TypeScript製の多くのフレームワークでは、実装したAPIサーバーからOpenAPIを出力するアプローチが採用されています。しかし、OpenAPI定義を元に、サーバーのテンプレートコードを自動生成するアプローチは多くはありません。そこで、OpenAPI定義を読み込み、Fastifyのコントローラー層の実装を自動生成するツールを作成しました。
